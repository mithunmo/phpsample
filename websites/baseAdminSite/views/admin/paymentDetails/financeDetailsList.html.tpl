<table class="payformtable" style="width:100%; overflow:auto;">
        ...
        <tbody>
            {foreach $oFinanceList['paymentList'] as $payment }
            <tr>
...
{if $oFinanceList['paymentPagination']['TotalRows'] > $oFinanceList['paymentPagination']['RowsLimit']}       
    <div class="paynavigation">
            {assign var=nextPage value=($oFinanceList['paymentPagination']['PageNo'] + 1)}
            {assign var=prevPage value=($oFinanceList['paymentPagination']['PageNo'] - 1)}
            {assign var=lastPage value=(ceil($oFinanceList['paymentPagination']['TotalRows']/$oFinanceList['paymentPagination']['RowsLimit']))}

            {if $oFinanceList['paymentPagination']['PageNo'] == 1 }
                <a class="first" ></a>
                <a class="previous" ></a>
            {else}
                <a class="first" href="/admin/paymentDetails/{$oController->getAction()}?{$rawDaoSearchQuery}&PageNo=1&RowsLimit={$oFinanceList['paymentPagination']['RowsLimit']}"></a>
                <a class="previous" href="/admin/paymentDetails/{$oController->getAction()}?{$rawDaoSearchQuery}&PageNo={$prevPage}&RowsLimit={$oFinanceList['paymentPagination']['RowsLimit']}"></a>
            {/if}
            {if $lastPage == $oFinanceList['paymentPagination']['PageNo'] }
                <a class="next" </a>
                <a class="last" ></a>
            {else}
                <a class="next" href="/admin/paymentDetails/{$oController->getAction()}?{$rawDaoSearchQuery}&PageNo={$nextPage}&RowsLimit={$oFinanceList['paymentPagination']['RowsLimit']}"></a>
                <a class="last" href="/admin/paymentDetails/{$oController->getAction()}?{$rawDaoSearchQuery}&PageNo={$lastPage}&RowsLimit={$oFinanceList['paymentPagination']['RowsLimit']}"></a>
            {/if}
    </div>
{/if}
