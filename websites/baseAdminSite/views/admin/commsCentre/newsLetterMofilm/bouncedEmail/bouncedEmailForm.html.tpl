{if $oController->getAction() == 'editObject' || $oController->getAction() == 'newObject'}
	<div class="hidden"><input type="hidden" name="PrimaryKey" value="{$oController->getPrimaryKey()|default:0}" /></div>
	<table class="data">
		<tbody>
			<tr>
				<th>{t}ID{/t}</th>
				<td><input type="text" name="ID" value="{$oObject->getID()}" /></td>
			</tr>
			<tr>
				<th>{t}Email Address{/t}</th>
				<td><input type="text" name="EmailAddress" value="{$oObject->getEmailAddress()}" /></td>
			</tr>
			<tr>
				<th>{t}Error Description{/t}</th>
				<td><input type="text" name="ErrorDescription" value="{$oObject->getErrorDescription()}" /></td>
			</tr>
			<tr>
				<th>{t}Sender Address{/t}</th>
				<td><input type="text" name="SenderAddress" value="{$oObject->getSenderAddress()}" /></td>
			</tr>
		</tbody>
	</table>
{elseif $oController->getAction() == 'deleteObject'}
	<p>{t}Are you sure you want to delete record ID &quot;{$oController->getPrimaryKey()}&quot;?{/t}</p>
{/if}