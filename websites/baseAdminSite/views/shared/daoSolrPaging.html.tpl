{if $totalObjects >= $limit && $offset >= 0}
	<tr>
		...
		<td class="alignRight">
			...
			<a href="{$daoUriView}?Offset={$oModel->getSolrLastPageOffset($limit)}&amp;Limit={$limit}{if $daoSearchQuery}&amp;{$daoSearchQuery}{/if}" title="{t}Last Page{/t}"><img src="{$themeicons}/32x32/result-set-last.png" alt="{t}Last Page{/t}" class="icon" /></a>
			{else}
