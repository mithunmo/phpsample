...
{assign var=searchProductID value=$oController->getProductQuery()}
	<div id="body">
		<div class="container">
			...
			<h2>{t}Videos : Search{/t} {if $oResults->getSearchInterface()->getUserID()} : {t}User ID :{/t} {$oResults->getSearchInterface()->getUserID()}{/if}</h2>
			...
			{if $searchDisplay == 'list'}
				{include file=$oView->getTemplateFile('resultList')}
			{else}
				{include file=$oView->getTemplateFile('resultGrid')}
			{/if}
		</div>
	</div>
