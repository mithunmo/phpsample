...
class pmController extends mvcController {
	...
	function launch() {
		switch ( $this->getAction() ) {
			...
			case self::ACTION_SENT:
				...
				$this->getMenuItems()
					->addItem(new mvcControllerMenuItem($this->buildUriPath(self::ACTION_NEW), 'New Message', 'mail-message-new', 'New Message', false, mvcControllerMenuItem::PATH_TYPE_URI, false))
	...
	protected function readMessage() {
		...
		if ( $this->getModel()->fetchInboxMessage($messageID) ) {
			...
			$this->getMenuItems()
				...
				->addItem(new mvcControllerMenuItem($this->buildUriPath(self::ACTION_FORWARD, $messageID), 'Forward', 'mail-forward', 'Forward', false, mvcControllerMenuItem::PATH_TYPE_URI, false))
				->addItem(new mvcControllerMenuItem($this->buildUriPath(self::ACTION_INBOX), 'Inbox', 'mail-folder-inbox', 'Inbox', false, mvcControllerMenuItem::PATH_TYPE_URI, false));
	...
	/**
	 * Performs a simple search for users
	 * 
	 * @return void
	 */
	protected function search() {
		if ( $this->getRequest()->isAjaxRequest() ) {
			...
			if ( strlen($data['tag']) >= 3 ) {
				$oSearch = new mofilmUserSearch();
				$oSearch->setUser($this->getRequest()->getSession()->getUser());
				$oSearch->setKeywords($data['tag']);
				$oSearch->setLimit(100);
				$oResults = $oSearch->search();
			} else {
				$oResults = new mofilmUserSearchResult(array(), 0, new mofilmUserSearch());
			}
			$oView = new pmView($this);
			$oView->showSearchResponse($oResults);
		} else {
			$this->redirect($this->buildUriPath(self::ACTION_INBOX));
		}
	}
