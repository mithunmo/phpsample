 ...
 * typeLeafModel class
 * 
 * Provides the "typeLeaf" page
 * 
 * @package websites_baseAdminSite
 * @subpackage controllers
 * @category typeLeafModel
 */
class typeLeafModel extends momusicTypeLeaf implements mvcDaoModelInterface {
	...
	function getObjectList($inOffset = null, $inLimit = 30) {
		return momusicTypeLeaf::listOfObjects($inOffset, $inLimit);
	}
	...
	function getTotalObjects() {
		/**
		 * @todo change database and table to the ones required for typeLeafModel
		 */
		$query = '
	...
	function getMusicType() {
		systemLog::message("hree".$this->getRootID());
		
		return momusicTypeLeaf::listOfObjectsByRoot($this->getRootID());
	}
}