...
class musicModel extends momusicWork {
	...
	function solrSearch() {
			...
			$oKeyword->setStart($this->getOffset());
                        $oKeyword->setCatalog($this->getCatalog());
	...
	function getFilterDefined($inKey) {
        ...
        function getBrandName($inBrandID){
            return mofilmSource::getInstance($inBrandID)->getEvent()->getName();
        }
        
        function getTrackInfo($inTrackID){
            return momusicWork::getInstance($inTrackID);
        }
        
        function getCoverImage($inCoverID){
            $imagePath = basename(momusicCoverimage::getInstance($inCoverID)->getPathImage());
            return "/resources/momusic/featuredArtist/" . $imagePath;
        }
        
        function getArtistImage($inImage){
            $imagePath = basename($inImage);
            return "/resources/momusic/featuredArtist/" . $imagePath;
	...
	function getBrandMusicID() {
		...
		$oKeyword = new momusicKeywordSearch();
		$oKeyword->setStart($this->getOffset());
                $oKeyword->setBrandMusiciD($this->getBrandMusicID());
		$oKeyword->briefMusicSearch();                
		$oResult =  $oKeyword->getResultList();
		return $oResult;
            
        }        
        
        function getOpenBriefs(){
            $myXMLData = file_get_contents("http://54.164.141.111/openSources/activeBriefs.xml");
            $xml=simplexml_load_string($myXMLData);
            $json = json_encode($xml);
            $array = json_decode($json,TRUE);
            $brief = $array["item"];
            $this->_OpenBrief = $brief;
        }
        
        function isBriefOpen($sourceID){
            $brief = $this->_OpenBrief;
            foreach( $brief as $item ){
                if($item["sourceID"] == $sourceID ){
                    systemLog::message($item["sourceID"]);                    
                    return true;
                    break;
                }
            }
        ...
        function getBriefName(){
            return mofilmSource::getInstance(momusicBrandmusic::getInstance($this->getBrandMusicID())->getBrandID())->getName();
        }
