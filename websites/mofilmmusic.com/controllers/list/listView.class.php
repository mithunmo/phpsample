...
class listView extends mvcView {
	...
	function showMoodListPage() {
		$this->setCacheLevelNone();
                systemLog::message("mood".$this->getModel()->getMoodList());
		if ( $this->getModel()->getMoodList() ) {
                        if ( $this->getModel()->getCatalog() != "mozayic"){
                            $this->getEngine()->assign("oList", $this->getModel()->getMoodList());
                            $this->getEngine()->assign("id", $this->getModel()->getID());
                        } else {
                            $this->getEngine()->assign("catalog", $this->getModel()->getCatalog());
                            $this->getEngine()->assign("oList", $this->getModel()->getMoodList());
                            $this->getEngine()->assign("id", $this->getModel()->getID());                            
                        }
			$this->render($this->getTpl('mood'));
                        ...
                        if ( $this->getModel()->getCatalog() == "mozayic" ){
        			$this->getController()->redirect("/music/solrSearch/?keyword=" . urlencode(stripcslashes($this->getModel()->getMood())));                                                
                        } else {
        			$this->getController()->redirect("/music/solrSearch/?keyword=" . urlencode(momusicType::getInstance($this->getModel()->getID())->getName()));                    
                        }
		}
	}
