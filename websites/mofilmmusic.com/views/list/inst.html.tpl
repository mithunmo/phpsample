		...
		<div>
			...
			{if $id}
				<div style="background-image:url(/themes/momusic/images/title_back_main.gif);">
					<div style="background-image:url(/themes/momusic/images/title_back_gm.gif);height:35px;padding-top:10px;float:left;padding-left:5px;">
						<strong><a href="/list/instrument">Instrument - </a></strong>{$oModel->getParentName($id)}</strong>
					</div>
			...
			{else}
				...
				{foreach $oList as $oObject}
					...
					{math assign="three" equation="$cnt%3"}
					{if $cnt == 1}
						<div  class="moodraw3"><a href="/list/instrument/{$oObject->getID()}">{$oObject->getName()}</a></div>
						<div  class="moodraw_spacer2">&nbsp;</div>
						{math assign="cnt" equation="$cnt+1"}					
					{else if $cnt == 2}
						<div  class="moodraw2" ><a href="/list/instrument/{$oObject->getID()}">{$oObject->getName()}</a></div>
						<div  class="moodraw_spacer">&nbsp;</div>
						{math assign="cnt" equation="$cnt+1"}					
					{else}
						<div class="moodraw1"><a href="/list/instrument/{$oObject->getID()}">{$oObject->getName()} </a></div>
						<div  class="moodraw_spacer">&nbsp;</div>
						{assign var="cnt" value=1}
					{/if}							
				{/foreach}	
