...
{else}
	...
	{foreach $oList as $oObject}
		{if $cnt == 1}
					<div  class="moodraw3"><a href="/list/style/{$oObject->getID()}">{$oObject->getName()}</a></div>
					<div  class="moodraw_spacer2">&nbsp;</div>
					{math assign="cnt" equation="$cnt+1"}
		{else if $cnt == 2}
					<div  class="moodraw2" ><a href="/list/style/{$oObject->getID()}">{$oObject->getName()}</a></div>
					<div  class="moodraw_spacer">&nbsp;</div>
					{math assign="cnt" equation="$cnt+1"}
		{else}
					<div class="moodraw1"><a href="/list/style/{$oObject->getID()}">{$oObject->getName()}</a></div>
					<div  class="moodraw_spacer">&nbsp;</div>
					{assign var="cnt" value=1}
		{/if}		
	{/foreach}	
