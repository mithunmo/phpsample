...
<!-- Content Starts --> 
	...
	<div style="width:740px;float:right;  height:inherit;">
		<div>
                        <h2> MOFILM Recommended tracks for {$oModel->getBriefName()} brief </h2>
			...
			<div id="navlink" style="font-size:10px;"> 
				<div style="float:left;width:300px;">
					...
					{if ($offset-$limit) >= 0}
                                        {assign var='lprev' value=$offset-$limit}						
                                        
						<a style="cursor: pointer;" href="/music/brief?brandMusicID={$brandMusicID}&offset={$lprev}&limit={$limit}"  title="{t}Previous{/t}"><img src="/themes/mofilm/images/icons/22x22/prev.png"></a>
					{else}
				...
				{assign var='lim' value=$offset+$limit}	
				
				<span id="totalb">Viewing {$o} - {$lim} of {$totalObjects} Results</span>
				{else}
				...
				<div style="float:right;width:250px;padding-right:5px;text-align:right;">
					{if $offset+$limit < $totalObjects}
						<a style="cursor: pointer;" href="/music/brief?brandMusicID={$brandMusicID}&Offset={$lim}&Limit={$limit}"  title="{t}Next{/t}"><img src="/themes/mofilm/images/icons/22x22/next.png"></a>
					{else}
				...
				<table width="740" height="50" border="0" align="center" cellpadding="0" cellspacing="3">
					{foreach $oResults as $oMusic}						
						<tr>
							...
							<td width="150px">{$oMusic->s_track}<br /> <a href="/music/solrSearch?artist={$oMusic->s_artist}"><strong>{$oMusic->s_artist}</strong></a></td>										
