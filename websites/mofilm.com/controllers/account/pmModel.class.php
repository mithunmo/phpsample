...
class pmModel extends mvcModelBase {
	...
	function sendMessage(array $data) {
		...
		if ( !isset($data['MessageAction']) || $data['MessageAction'] != 'reply' ) {
			throw new mvcModelException('You are only permitted to reply to messages.');
		}
	 ...
	 * @throws mvcModelException
	 */
	function postMessage(array $data) {
		if ( !isset($data['Recipient']) || !is_array($data['Recipient']) || count($data['Recipient']) < 1 ) {
			throw new mvcModelException('No recipient was set, please ensure at least 1 (one) recipient is set.');
		}
	...
	function sendMessageNotification(mofilmUserPrivateMessage $inMessage) {
		if ( $inMessage->getSender()->getClientID() == 1 || $inMessage->getRecipient()->getClientID() == 1 ) {
			if ( $inMessage->getRecipient()->getParamSet()->getParam('PrivateMessageAlerts', 1) ) {
				...
				$oQueue = commsOutboundManager::newQueueFromApplicationMessageGroup(
					0, mofilmMessages::MSG_GRP_CLIENT_PRIVATE_MESSAGE, $this->getRequest()->getDistributor()->getSiteConfig()->getI18nDefaultLanguage()->getParamValue()
				);
				...
				$oQueue = commsOutboundManager::newQueueFromApplicationMessageGroup(
					0, mofilmMessages::MSG_GRP_CREW, $this->getRequest()->getDistributor()->getSiteConfig()->getI18nDefaultLanguage()->getParamValue()
				);
				commsOutboundManager::setOriginatorInMessageStack($oQueue, $inMessage->getSender()->getEmail());
				commsOutboundManager::setCustomerInMessageStack($oQueue, $inMessage->getRecipient()->getID());
				commsOutboundManager::setRecipientInMessageStack($oQueue, $inMessage->getRecipient()->getEmail());
				commsOutboundManager::replaceDataInMessageStack($oQueue, array('%mofilm.username%'), array($inMessage->getSender()->getFullname()));
				if ( $oQueue->send() ) {
					return true;
				} else {
					throw new mvcModelException($oQueue->getLastException()->getMessage());
	 ...
	 * 
	 */
	function setContact($inContact) {
		if ( $inContact !== $this->_Contact ) {
			$this->_Contact = $inContact;
			$this->setModified();
		}
		return $this;		
	}	
