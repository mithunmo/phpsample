...
class videoUploadDaemon extends cliDaemon {
	...
	function execute() {
		...
		do {
			...
			for ( $i=14664; $i>= 14051; $i--) {

				systemLog::message("Processing the movieID " . $i);

				$oMovie = mofilmMovieManager::getInstanceByID($i);
				...
				if ( $return ) {
					...
					$oMofilmUploadStatus->setMovieID($i);
					$oMofilmUploadStatus->setVideoCloudID($return);
					$oMofilmUploadStatus->setStatus(mofilmUploadStatus::STATUS_ENCODING);
					$oMofilmUploadStatus->save();
				} else {
				    systemLog::message("-- Failed to send to Video Cloud -- ".$i);
				}
			}
