...
class videoUploadDaemon extends cliDaemon {
	...
	function execute() {
		...
		do {
			...
			try {
				$oMofilmUploadQueue = mofilmUploadQueue::getMovieFromQueue();
				if ( $oMofilmUploadQueue ) {
					...
					if ( $return ) {
						...
						$oMofilmUploadStatus->setUpdateDate(date('Y-m-d H:i:s'));
				...
				systemLog::message('--- in VideoUpload catch ---');
				systemLog::message($e);
				$loop = true;
			}
	...
	function uploadToVideoCloud($Id, $shortDesc, $filename) {
	    try {
		    ...
		    $readapi = system::getConfig()->getParam('mofilmvideoupload', 'readapi1')->getParamValue();
		    $writeapi = system::getConfig()->getParam('mofilmvideoupload', 'writeapi1')->getParamValue();
