...
systemLog::message('--------------------------------------------------');
systemLog::message('Initialising SiteMap Generator :: Closed Competitions Daemon');
...
class videoUploadDaemon extends cliDaemon {
	...
	function execute() {
		...
		do {
			...
			try {
				$oOpenCompetitions = mofilmEvent::listOfObjects();
				...
				foreach ($oOpenCompetitions as $oOpenCompetition) {
				    systemLog::message($oOpenCompetition);
					if ( $oOpenCompetition->getWebpath() && $oOpenCompetition->getEndDate() != '0000-00-00 00:00:00' && $oOpenCompetition->getEndDate() < date('Y-m-d H:i:s')) {
					    systemLog::message('-- if --');
				...
				$xmlElement->asXML('/var/www/html/mofilmcake/app/webroot/sitemap/closed_competitions.xml');
				$loop = false;
