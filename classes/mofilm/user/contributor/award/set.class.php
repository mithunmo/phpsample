 ...
 * @package mofilm
 * @subpackage movie
 * @category mofilmUserContributorAwardSet
 * @version $Rev: 10 $
 */
 ...
 * mofilmUserContributorAwardSet
 * 
 * Manages contributor awards, optionally only loading awards for the specified event.
 * 
 * @package mofilm
 * @subpackage movie
 * @category mofilmUserContributorAwardSet
 */
class mofilmUserContributorAwardSet extends baseSet implements systemDaoInterface {
	...
	function load() {
		if ( $this->_UserID ) {
			$oContributor = mofilmContributor::getInstanceByEmail(mofilmUserManager::getInstanceByID($this->getUserID())->getEmail());
			if ( $oContributor instanceof mofilmContributor ) {
				$values = array(
					':UserID' => $oContributor->getID(),
				...
				$query = '
					SELECT movieAwards.*
					 ...
					 WHERE  movieAwards.movieID in ( SELECT movieID FROM '.system::getConfig()->getDatabase('mofilm_content').'.movieContributors WHERE contributorID = :UserID )
