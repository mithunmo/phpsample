<?php
/**
 * mofilmCommsNewsletterFilterVideoratingcca
 *
 * Stored in mofilmCommsNewsletterFilterVideoratingcca.class.php
 *
 * @author Mithun Mohan
 * @copyright Mofilm (c) 2009-2010
 * @package mofilm
 * @subpackage mofilmCommsNewsletterFilterVideoratingcca
 * @category mofilmCommsNewsletterFilterVideoratingcca
 * @version $Rev: 73 $
 */
class mofilmCommsNewsletterFilterVideoratingcca extends mofilmCommsNewsletterFilterlist {
		...
		if ( $oStmt->execute() ) {
			foreach ( $oStmt as $row ) {
				if ( mofilmCommsCcaEmails::getCCARecordByMovieandEventID($row["movieID"], $this->getNewsletterData()->getParamSet()->getParam(mofilmCommsNewsletterdata::PARAM_NL_EVENTID)) ) {
					...
					/* Adding the record if not alredy exists in movieAward table */
					$oMovieAward = mofilmMovieAward::getInstanceByMovieEventAndType($row["movieID"], $row["eventID"], mofilmMovieAward::TYPE_SHORTLISTED);
					if ( !$oMovieAward instanceof mofilmMovieAward ) {
						$oMofilmMovieAward = mofilmMovieAward::factory($row["ID"], $row["movieID"], $row["eventID"], $row["sourceID"], 0, mofilmMovieAward::TYPE_SHORTLISTED);
						$oMofilmMovieAward->save();
						$oMofilmMovieAward = null;
						$oMofilmCommsCCA = null;
						unset ($oMofilmMovieAward,$oMofilmCommsCCA);
					}
