 ...
 * mofilmUploadQueue
 *
 * Stored in mofilmUploadQueue.class.php
 *
 * @author Pavan Kumar
 * @copyright Mofilm (c) 2009-2010
 * @package mofilm
 * @subpackage mofilmUploadQueue
 * @category mofilmUploadQueue
 * @version $Rev: 840 $
 */


/**
 * mofilmUploadQueue Class
 *
 * Provides access to records in mofilm_content.uploadQueue
 *
 * Creating a new record:
 * <code>
 * $oMofilmUploadQueue = new mofilmUploadQueue();
 * $oMofilmUploadQueue->setID($inID);
 * $oMofilmUploadQueue->setMovieID($inMovieID);
 * $oMofilmUploadQueue->setBulkID($inBulkID);
 * $oMofilmUploadQueue->setUserID($inUserID);
 * $oMofilmUploadQueue->setQueued($inQueued);
 * $oMofilmUploadQueue->setProfile($inProfile);
 * $oMofilmUploadQueue->setStatus($inStatus);
 * $oMofilmUploadQueue->setSent($inSent);
 * $oMofilmUploadQueue->setNotes($inNotes);
 * $oMofilmUploadQueue->save();
 * </code>
 *
 * Accessing a record by primary key on constructor:
 * <code>
 * $oMofilmUploadQueue = new mofilmUploadQueue($inID);
 * </code>
 *
 ...
 * $oMofilmUploadQueue = new mofilmUploadQueue();
 * $oMofilmUploadQueue->setID($inID);
 * $oMofilmUploadQueue->load();
 * </code>
 *
 * Accessing a record by instance:
 * <code>
 * $oMofilmUploadQueue = mofilmUploadQueue::getInstance($inID);
 * </code>
 ...
 * @category mofilmUploadQueue
 */
class mofilmUploadQueue implements systemDaoInterface, systemDaoValidatorInterface {
	...
	/**
	 * Container for static instances of mofilmUploadQueue
	 *
	 * @var array
	 * @access protected
	 * @static
	...
	/**
	 * Returns a new instance of mofilmUploadQueue
	 *
	 * @param integer $inID
	 * @return mofilmUploadQueue
	 */
	...
	/**
	 * Get an instance of mofilmUploadQueue by primary key
	 *
	 * @param integer $inID
	 * @return mofilmUploadQueue
	 * @static
	...
	/**
	 * Returns an array of objects of mofilmUploadQueue
	 *
	...
	public static function listOfObjects($inOffset = null, $inLimit = 30) {
		...
		$query = '
			SELECT ID, movieID, bulkID, userID, queued, profile, status, sent, notes
			  FROM '.system::getConfig()->getDatabase('mofilm_content').'.uploadQueue
			 WHERE 1';
	 ...
	 * @static
	 */
	public static function getMovieFromQueueDesc() {
		/*
		 * Holds values to be assigned during query execution. Values do not need
		 * to be escaped because they are injected into named place-holders in the
		 * prepared query. Add items using $values[':PlaceHolder'] = $value;
	...
	function save() {
		...
		if ( $this->isModified() ) {
			...
			if ( $this->_Modified ) {
				$query = '
				INSERT INTO '.system::getConfig()->getDatabase('mofilm_content').'.uploadQueue
	...
	function delete() {
		$query = '
			DELETE FROM '.system::getConfig()->getDatabase('mofilm_content').'.uploadQueue
			WHERE
				ID = :ID
			LIMIT 1';
	 ...
	 * mofilmUploadQueue::PRIMARY_KEY_SEPARATOR.
 	 *
	 * @param string $inKey
	 * @return mofilmUploadQueue
  	 */
